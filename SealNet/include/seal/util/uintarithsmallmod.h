...
namespace seal
{
    namespace util
    {
        ...
        inline std::uint64_t barrett_reduce_128(const std::uint64_t *input, 
            const SmallModulus &modulus)
        {
#ifdef SEAL_DEBUG
            if (input == nullptr)
            ...
            return tmp3 - (modulus.value() & static_cast<uint64_t>(
        ...
        inline void modulo_uint_inplace(std::uint64_t *value, 
            int value_uint64_count, const SmallModulus &modulus)
        {
#ifdef SEAL_DEBUG
        ...
        inline std::uint64_t modulo_uint(const std::uint64_t *value, 
            int value_uint64_count, const SmallModulus &modulus, MemoryPool &pool)
        {
#ifdef SEAL_DEBUG
            if (value == nullptr && value_uint64_count > 0)
            {
        ...
        // Try to find a primitive degree-th root of unity modulo small prime modulus, 
        ...
        // small prime modulus, where degree must be a power of two.
        ...
        void divide_uint_uint_mod_inplace(uint64_t *numerator, 
            const SmallModulus &modulus, int uint64_count, uint64_t *quotient, 
            MemoryPool &pool);
    }
}
