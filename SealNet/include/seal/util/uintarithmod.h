...
namespace seal
{
    namespace util
    {
        void modulo_uint_inplace(std::uint64_t *value, int value_uint64_count, 
            const Modulus &modulus, MemoryPool &pool);

        inline void modulo_uint(const std::uint64_t *value, int value_uint64_count, 
        ...
        inline void increment_uint_mod(const std::uint64_t *operand, 
            const std::uint64_t *modulus, int uint64_count, std::uint64_t *result)
        {
#ifdef SEAL_DEBUG
            if (operand == nullptr)
            {
        ...
        inline void multiply_uint_uint_mod(const std::uint64_t *operand1, 
            const std::uint64_t *operand2, const Modulus &modulus, 
        ...
        // Find if root is a primitive degree-th root of unity modulo prime_modulus, 
        // where degree must be a power of two.
        bool is_primitive_root(const std::uint64_t *root, std::uint64_t degree, 
            const Modulus &prime_modulus, MemoryPool &pool);

        // Try to find a primitive degree-th root of unity modulo prime_modulus, 
        // where degree must be a power of two.
        bool try_primitive_root(std::uint64_t degree, const Modulus &prime_modulus, 
            std::uint64_t *destination, MemoryPool &pool);

        // Try to find the smallest (as integer) primitive degree-th root of unity modulo 
        // prime_modulus, where degree must be a power of two.
        bool try_minimal_primitive_root(std::uint64_t degree, const Modulus &prime_modulus, 
            std::uint64_t *destination, MemoryPool &pool);
    
        void exponentiate_uint_mod(const std::uint64_t *operand, 
