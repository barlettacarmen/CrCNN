...
namespace seal
{
    ...
    */
    ...
    //std::vector<uint64_t> compute_lift_uint64(long p, long n); //Generate lifting polynomial f_n(X+O(p^n)) = X+O(p^{n+1})

    //std::vector<uint64_t> compute_remainlsd_uint64(long p, long n); //Generate remaindlsd polynomial f_n(X) = X - [X]_p for x in Z_{p^n}
