...
namespace seal
{
    namespace util
    {
        ...
        std::string uint_to_dec_string(const std::uint64_t *value, 
            int uint64_count, MemoryPool &pool);

        inline void hex_string_to_uint(const char *hex_string, 
            int char_count, int uint64_count, std::uint64_t *result)
        {
#ifdef SEAL_DEBUG
            ...
            if (get_hex_string_bit_count(hex_string, char_count) > 
                uint64_count * bits_per_uint64)
            {
                throw std::invalid_argument("hex_string");
            }
#endif
        ...
        inline Pointer allocate_uint(std::int64_t uint64_count, MemoryPool &pool)
        {
#ifdef SEAL_DEBUG
            if (uint64_count < 0)
            {
        ...
        inline void set_uint(std::uint64_t value, int uint64_count, 
            std::uint64_t* result)
        {
#ifdef SEAL_DEBUG
            if (uint64_count <= 0)
        ...
        inline bool is_equal_uint(const std::uint64_t *value, 
            int uint64_count, std::uint64_t scalar)
        {
#ifdef SEAL_DEBUG
            if (value == nullptr)
            {
        ...
        inline bool is_bit_set_uint(const std::uint64_t *value, 
            int uint64_count, int bit_index)
        {
#ifdef SEAL_DEBUG
            if (value == nullptr)
            {
        ...
        inline void set_bit_uint(std::uint64_t *value, int uint64_count, 
            int bit_index)
        {
#ifdef SEAL_DEBUG
            if (value == nullptr)
        ...
        inline int get_significant_bit_count_uint(const std::uint64_t *value, 
            int uint64_count)
        {
#ifdef SEAL_DEBUG
            if (value == nullptr && uint64_count > 0)
        ...
        inline int get_significant_uint64_count_uint(
            const std::uint64_t *value, int uint64_count)
        {
#ifdef SEAL_DEBUG
            if (value == nullptr && uint64_count > 0)
            {
        ...
        inline void set_uint_uint(const std::uint64_t *value, 
            int value_uint64_count, int result_uint64_count, 
            std::uint64_t *result)
        {
#ifdef SEAL_DEBUG
        ...
        inline int get_power_of_two_minus_one_uint(
            const std::uint64_t *operand, int uint64_count)
        {
#ifdef SEAL_DEBUG
            if (operand == nullptr && uint64_count > 0)
            {
        ...
        inline void filter_highbits_uint(std::uint64_t *operand, 
            int uint64_count, int bit_count)
        {
#ifdef SEAL_DEBUG
            if (operand == nullptr && uint64_count > 0)
            {
            ...
            *operand++ &= (1ULL << subbit_index) - 1;
            for (int long_index = uint64_index + 1; long_index < uint64_count; 
                long_index++)
            {
                *operand++ = 0;
            }
        ...
        inline ConstPointer duplicate_uint_if_needed(const std::uint64_t *uint, 
            int uint64_count, int new_uint64_count, bool force, MemoryPool &pool)
        {
#ifdef SEAL_DEBUG
        ...
        inline int compare_uint_uint(const std::uint64_t *operand1, 
            const std::uint64_t *operand2, int uint64_count)
        {
#ifdef SEAL_DEBUG
        ...
        inline bool is_greater_than_or_equal_uint_uint(
            const std::uint64_t *operand1, int operand1_uint64_count, 
            const std::uint64_t *operand2, int operand2_uint64_count)
        {
            return compare_uint_uint(operand1, operand1_uint64_count, operand2, 
                operand2_uint64_count) >= 0;
        }
